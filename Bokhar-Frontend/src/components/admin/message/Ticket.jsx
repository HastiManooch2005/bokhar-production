// Ticket.jsx — صفحه چت تیکت (فول پیج) با API
import { useState, useRef, useEffect, useCallback } from "react";
import { useNavigate, useParams } from "react-router-dom";
import Sidebar from "../Sidebar";
import { getTicketDetail, replyToTicket } from "../../../api/ticketApi";
...
export default function Ticket() {
  ...
  const sendMessage = async (content, type, extra = {}) => {
    ...
    try {
      ...
      const response = await replyToTicket(id, data);
      setTicket(response);
      setReplyText("");
    } catch (err) {
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-1 text-xs text-slate-500 dark:text-gray-400">
                ...
                <span>{ticket?.user_full_name || ticket?.user_name || "ناشناس"}</span>
                ...
                >
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <span className="text-xs font-semibold opacity-70">
                      {msg.is_admin ? "پشتیبان" : (ticket?.user_full_name || "کاربر")}
                    </span>
