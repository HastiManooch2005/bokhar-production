...
const usePasswordManager = ({user, refreshUser, API_BASE, getCSRFToken}) => {
    ...
    const sendOTP = useCallback(async () => {
        ...
        setError("");
        try {
            const res = await fetch(`${API_BASE}/send/otp/password/`, {
                method: "POST",
                headers: {
    ...
    const handlePasswordChange = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) {
                const msg =
                    result.detail || result.non_field_errors?.[0] || result.old_password?.[0] || result.password?.[0] || result.password2?.[0] || result.otp_code?.[0] ||
...
export default function PasswordSection() {
    ...
    const {user, refreshUser} = {
        user: {has_password: true, phone: "09123456789"}, refreshUser: () => {
        }
    };
                                ...
                                {showRules && (
                                    ...
                                    >
                                        ...
                                        <ul className="space-y-2 text-xs">
                                            {[
                                                {
                                                    test: /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password.new),
                                                    label: "کاراکتر خاص"
                                                },
                                                {
                                                    test: /[a-z]/.test(password.new) && /[A-Z]/.test(password.new),
                                                    label: "حروف بزرگ و کوچک"
                                                },
                                                ...
                                                <li key={idx} className="flex items-center gap-2">
                                                    ...
                                                    />
                                                    <span
                                                        className={item.test ? "text-emerald-600 dark:text-emerald-400 font-medium" : "text-gray-500 dark:text-gray-400"}>
                            {item.label}
                          </span>
                                                </li>
                                            ))}
